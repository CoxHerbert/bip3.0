<template>
  ...
  <el-dialog :title="submitTitle" append-to-body v-model="openSubmit" :width="modalWidth">
    <avue-form
...
watch(
  ...
  async newVal => {
    ...
    if (Array.isArray(newVal)) {
      ...
      ids = [newVal?.id].filter(Boolean); // 确保 id 不为空
  ...
  newVal => {
    nextTick(() => {
      newVal.forEach(row => {
        ...
        if (newSelectedRow) {
          const findIndex = infoRows.value.findIndex(infoRow => infoRow.id === newSelectedRow.id);
          if (findIndex > -1) {
...
const handleEdit = row => {
  ...
  if (!Array.isArray(row?.areaCode) && !!row?.areaCode) {
    formData.value.areaCode = row?.areaCode.split(',');
  }
...
const submitForm = async (form, done) => {
  try {
    if (props?.params) form = { ...form, ...props.params };
    console.log(form);
    if (Array.isArray(form?.areaCode)) {
      form.areaCode = form?.areaCode ? form?.areaCode.join(',') : null;
    }
...
const validateSelectionLimit2 = row => {
  return new Promise(resolve => {
    const isAlreadySelected = showSelectedRows.value.some(item => item.id === row.id);
