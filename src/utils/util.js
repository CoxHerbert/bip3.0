...
/**
 * 把形如
 *   http://host/xxxx/upload/selfResume/%E5%B3%B0%E6%A2%85...%20Jeep%20....docx
 * 或包含 &amp; 的字符串
 * 转成：
 *   /api/upload/selfResume/峰梅新能源员工&amp;供应商 Jeep 购车优惠方案.docx
 *
 * @param {string} input
 * @param {object} [opts]
 * @param {boolean} [opts.htmlEscapeAmp=true] 是否将 & 转成 &amp;（用于直接塞到 HTML 里显示）
 * @returns {string}
 */
export function toApiUploadPath(input, opts = {}) {
  const { htmlEscapeAmp = true } = opts;

  if (!input) return '';

  // 1) 先把 HTML 实体的 &amp; 还原为 &，以便 URL 解析
  const unescaped = String(input).replace(/&amp;/g, '&');

  // 2) 解析 URL（如果本身是相对路径，也兼容）
  let pathname = '';
  try {
    // new URL 需要绝对地址；相对的用个占位域名兜底
    const u = unescaped.startsWith('http')
      ? new URL(unescaped)
      : new URL(unescaped, 'http://placeholder.local');
    pathname = u.pathname || '';
  } catch {
    // 解析失败就当作纯路径
    pathname = unescaped;
  ...
  // 3) 只保留从 /upload/ 开始的部分（去掉租户/环境前缀）
  const hit = pathname.indexOf('/upload/');
  let pure = hit >= 0 ? pathname.slice(hit) : pathname;

  // 4) URL 解码（把 %E5%... 变成中文，把 %20 变成空格）
  try {
    pure = decodeURIComponent(pure);
  } catch {
    // 忍受部分不规范编码，忽略错误
  }

  // 5) 规范一下连续斜杠
  pure = pure.replace(/\/{2,}/g, '/');

  // 6) 加上 /api 前缀（如果没有的话）
  if (!pure.startsWith('/api/')) {
    pure = pure.startsWith('/') ? '/api' + pure : '/api/' + pure;
  }

  // 7) 需要在 HTML 中直接展示时，把 & 转为 &amp;
  if (htmlEscapeAmp) {
    pure = pure.replace(/&/g, '&amp;');
  }
