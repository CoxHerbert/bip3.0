<template>
  ...
  >
    <div v-loading="loading" class="content-role-bind-users">
      <dc-drag-panel leftInitWidth="0" :height="contentHeight">
        <template #right>
          <div class="right-box">
  ...
  methods: {
    ...
    doAction(action) {
      if (action === 'binddSelectData') {
        ...
        this.loading = true;
        Api.mes.basicConfig.technologyUserRelationBind(this.getRowData.id, ids).then(res => {
        ...
        const ids = rows.map(row => row.id);
        this.loading = true;
        Api.mes.basicConfig.technologyUserRelationCancelBind(this.getRowData.id, ids).then(res => {
          const { code, msg } = res.data;
          if (code === 200) {
            this.$message.success(msg);
    ...
    async getData(isRight) {
      const dataSend = {
        ...
        bindingStatus: !!isRight,
        technologyId: this.getRowData.id,
      };
      const res = await Api.mes.basicConfig.technologyUserRelationList(dataSend);
      const { code, data } = res.data;
      if (code === 200) {
        if (!isRight) {
