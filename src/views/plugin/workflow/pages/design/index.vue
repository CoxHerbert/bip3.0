                  ...
                  change: val => {
                    ...
                    if (val) {
                      getFormByKey({ formKey: val })
                        .then(res => {
                          // 将 proxy 替换为 this，然后使用 Function 构造函数执行
                          let formOption;
                          try {
                            // 将 content 中的 proxy 替换为 this
                            const content = res.data.data.content.replace(/proxy/g, 'this');
                            const formFunction = new Function('return (' + content + ')');
                            formOption = formFunction.call(this);
                          } catch (error) {
                            console.error('解析表单配置失败:', error);
                            formOption = { column: [], group: [] };
                          }
                          this.option = {
                            ...formOption,
                            menuBtn: false,
                        ...
                        .catch(err => {
                          ...
                          this.findObject(this.step1.option.group, 'form').display = false;
                        });
                    } else {
