    ...
    <el-radio-group v-model="radio">
      <el-radio v-for="(item, index) in detailData" :value="item" :key="index">
        <div class="history-title">
          ...
          <div class="create-time">核价人：{{ item.userName }}</div>
          <div class="create-time">采购数量：{{ Number(item.qty).toFixed(2) }}</div>
          <div class="create-time">
            零件单价：{{ Number(item.currentOutsourcePrice).toFixed(2) }}
          </div>
        </div>
        <el-table
          border
          :data="item?.historyTechnologyItems || []"
          style="height: 100%"
          size="small"
        >
...
export default {
  ...
  methods: {
    ...
    getData(materialNumber) {
      this.loading = true;
      Api.system.OutsourceQuotation.getHistoryProcess(materialNumber)
        .then(res => {
