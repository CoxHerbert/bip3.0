              ...
              >
                <template #default="scoped">
                  ...
                  <span
                    ...
                    v-if="child.type == 'clickable-text'"
                    @click="handleMaterialNumClick(scoped.row, props.row)"
                  >
                    {{ scoped.row[child.prop] }}
                  </span>
                  <span :style="child.customStyle ? child.customStyle(scoped.row) : {}" v-else>
                    {{
                      ...
                      ([
                        ...
                        parseFloat(scoped.row[child.prop]) === 0)
                        ? '-'
                        : scoped.row[child.prop]
                    }}
                  </span>
    ...
    this.getDictData().then(() => {
      ...
      this.searchConfig.searchItemConfig.paramType = {
        erpUp: {
          ...
          props: { placeholder: '请选择单据类型', clearable: true },
        },
        billNo: {
          label: '单据编号',
          type: 'input',
          paramKey: 'billNo',
          props: { placeholder: '请输入单据编号', clearable: true },
        },
        purchaserName: {
          label: '采购人',
          type: 'input',
          paramKey: 'purchaserName',
          props: { placeholder: '请输入采购人', clearable: true },
        },
        supplierShortname: {
          label: '供应商',
          type: 'input',
          paramKey: 'supplierShortname',
        ...
        taskName: {
          ...
          paramKey: 'taskName',
          search: true,
          searchProps: { is: 'wf-select-dialog' },
          props: { objectName: 'user' },
        },
      };
    });
  },
